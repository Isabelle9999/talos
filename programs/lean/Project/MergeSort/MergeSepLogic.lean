...
private def MergeLoopInv
  ...
  ∃ i j : Nat,
    ...
    -- content: output at positions k₀..k-1 equals merge of processed left/right prefixes
    (∀ q, q < (i - i₀) + (j - j₀) →
      stA.mem.read32 (out_ptr + 4 * UInt32.ofNat (k₀ + q)) =
        ((List.merge
            ((wordsAt st_init.mem left_ptr n_left.toNat).drop i₀ |>.take (i - i₀))
            ((wordsAt st_init.mem right_ptr n_right.toNat).drop j₀ |>.take (j - j₀))
            (· ≤ ·))[q]?).getD 0) ∧
    -- memory bounds
          ...
          have hI_A : MergeLoopInv frame out_ptr left_ptr right_ptr n_left n_right n_out
            ...
            ⟨i + 1, j, by omega, by omega, hj_lo, hj_hi,
             ...
             fun q hq => by
               ...
               exact hright q hq,
             (by sorry), -- content: needs merge correctness lemma
