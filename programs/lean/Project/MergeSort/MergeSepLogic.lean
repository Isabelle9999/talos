...
set_option maxHeartbeats 800000 in
theorem main_merge_loop_spec_exit
    {m : Module} {env : HostEnv Unit}
    (st : Store Unit) (locals : Locals)
    (frame out_ptr left_ptr right_ptr n_left n_right n_out : UInt32)
    (i₀ j₀ k₀ : Nat)
    ...
    ∃ (N : Nat) (st' : Store Unit) (loc' : Locals),
      ...
      MergeLoopInv frame out_ptr left_ptr right_ptr n_left n_right n_out i₀ j₀ k₀ st st' loc' ∧
      (st'.mem.read32 (frame + 8)  = n_left ∨
       st'.mem.read32 (frame + 12) = n_right) := by
  -- strong induction on μ = (n_left - i) + (n_right - j)
  suffices key : ∀ n stA locA,
    ...
    · by_cases hlt_j : j < n_right.toNat
      · -- iteration: i < n_left, j < n_right
        ...
        · -- ── path A: left[i] ≤ right[j]: copy left[i] to out[k], i++, k++ ──
          ...
          -- IH at reduced measure: input is (stC_A, locA_out_A)
          -- IH at reduced measure: input is (stC_A, locA_out_A)
          obtain ⟨N_A, stR_A, locR_A, h_IH_A, h_IH_inv_A, hQ_A⟩ := IH _ hμ_A stC_A locA_out_A hI_A rfl
          -- Fuel composition: one body iteration at stA then IH fuel at stC_A
          have hbody_ne : exec f_A m stA locA mainMergeBody env ≠ .OutOfFuel := by
            simp [h_body_A]
          have hfuel_ne : exec N_A m stC_A locA_out_A [.block 0 0 [.loop 0 0 mainMergeBody]] env ≠ .OutOfFuel :=
            by rw [h_IH_A]; intro h; cases h
          have hbody_mono : exec (max f_A N_A) m stA locA mainMergeBody env = .Break 0 stC_A locA_out_A :=
            (exec_fuel_mono (Nat.le_max_left f_A N_A) hbody_ne).trans h_body_A
          have hblock_mono : exec (max f_A N_A + 1) m stC_A locA_out_A [.block 0 0 [.loop 0 0 mainMergeBody]] env =
              exec N_A m stC_A locA_out_A [.block 0 0 [.loop 0 0 mainMergeBody]] env :=
            exec_fuel_mono (by omega) hfuel_ne
          have hloop_single : ∀ F stT locT,
              exec F m stT locT [.loop 0 0 mainMergeBody] env =
              execOne F m stT locT (.loop 0 0 mainMergeBody) env := fun F stT locT => by
            ...
            | succ f =>
              ...
              · rfl
              all_goals rfl
          have hloop_eq : exec (max f_A N_A + 1) m stA locA [.loop 0 0 mainMergeBody] env =
              exec (max f_A N_A) m stC_A locA_out_A [.loop 0 0 mainMergeBody] env := by
            rw [hloop_single, hloop_single]
            conv_lhs => rw [execOne_loop_succ]
            simp only [hbody_mono, List.take_zero, List.nil_append, List.drop_zero]
            -- {locA_out_A with values := locA.values} = locA_out_A by rfl:
            -- locA_out_A = {locA with locals := ...}, so .values = locA.values definitionally
            rfl
          have heq : exec (max f_A N_A + 2) m stA locA [.block 0 0 [.loop 0 0 mainMergeBody]] env =
              exec (max f_A N_A + 1) m stC_A locA_out_A [.block 0 0 [.loop 0 0 mainMergeBody]] env := by
            rw [show max f_A N_A + 2 = max f_A N_A + 1 + 1 from rfl]
            conv_lhs => rw [exec_block_cons, hloop_eq]
            conv_rhs => rw [exec_block_cons]
            set discr := exec (max f_A N_A) m stC_A locA_out_A [.loop 0 0 mainMergeBody] env
            rcases discr with ⟨r', s'⟩ | ⟨n, r', s'⟩ | ⟨r', vs⟩ | ⟨r', msg⟩ | ⟨msg⟩ | _
            · simp [exec, locA_out_A, locA_out_locs]
            · cases n with | zero => simp [exec, locA_out_A, locA_out_locs] | succ k => rfl
            all_goals rfl
          refine ⟨max f_A N_A + 2, stR_A, locR_A, ?_, h_IH_inv_A, hQ_A⟩
          rw [heq, hblock_mono]; exact h_IH_A
        · -- ── path B: left[i] > right[j]: copy right[j] to out[k], j++, k++ ──
          let mem1_B := stA.mem.write32 (out_ptr + 4 * UInt32.ofNat k) right_j
          let mem2_B := mem1_B.write32 (frame + 12) (UInt32.ofNat j + 1)
          let mem3_B := mem2_B.write32 (frame + 16) (UInt32.ofNat k + 1)
          ...
          -- IH at reduced measure: input is (stC_B, locB_out_B)
          -- IH at reduced measure: input is (stC_B, locB_out_B)
          obtain ⟨N_B, stR_B, locR_B, h_IH_B, h_IH_inv_B, hQ_B⟩ := IH _ hμ_B stC_B locB_out_B hI_B rfl
          -- Fuel composition: one body iteration at stA then IH fuel at stC_B
          have hbody_ne : exec f_B m stA locA mainMergeBody env ≠ .OutOfFuel := by
            simp [h_body_B]
          have hfuel_ne : exec N_B m stC_B locB_out_B [.block 0 0 [.loop 0 0 mainMergeBody]] env ≠ .OutOfFuel :=
            by rw [h_IH_B]; intro h; cases h
          have hbody_mono : exec (max f_B N_B) m stA locA mainMergeBody env = .Break 0 stC_B locB_out_B :=
            (exec_fuel_mono (Nat.le_max_left f_B N_B) hbody_ne).trans h_body_B
          have hblock_mono : exec (max f_B N_B + 1) m stC_B locB_out_B [.block 0 0 [.loop 0 0 mainMergeBody]] env =
              exec N_B m stC_B locB_out_B [.block 0 0 [.loop 0 0 mainMergeBody]] env :=
            exec_fuel_mono (by omega) hfuel_ne
          have hloop_single : ∀ F stT locT,
              exec F m stT locT [.loop 0 0 mainMergeBody] env =
              execOne F m stT locT (.loop 0 0 mainMergeBody) env := fun F stT locT => by
            ...
            | succ f =>
              ...
              · rfl
              all_goals rfl
          have hloop_eq : exec (max f_B N_B + 1) m stA locA [.loop 0 0 mainMergeBody] env =
              exec (max f_B N_B) m stC_B locB_out_B [.loop 0 0 mainMergeBody] env := by
            rw [hloop_single, hloop_single]
            conv_lhs => rw [execOne_loop_succ]
            simp only [hbody_mono, List.take_zero, List.nil_append, List.drop_zero]
            rfl
          have heq : exec (max f_B N_B + 2) m stA locA [.block 0 0 [.loop 0 0 mainMergeBody]] env =
              exec (max f_B N_B + 1) m stC_B locB_out_B [.block 0 0 [.loop 0 0 mainMergeBody]] env := by
            rw [show max f_B N_B + 2 = max f_B N_B + 1 + 1 from rfl]
            conv_lhs => rw [exec_block_cons, hloop_eq]
            conv_rhs => rw [exec_block_cons]
            set discr := exec (max f_B N_B) m stC_B locB_out_B [.loop 0 0 mainMergeBody] env
            rcases discr with ⟨r', s'⟩ | ⟨n, r', s'⟩ | ⟨r', vs⟩ | ⟨r', msg⟩ | ⟨msg⟩ | _
            · simp [exec, locB_out_B, locB_out_locs]
            · cases n with | zero => simp [exec, locB_out_B, locB_out_locs] | succ k => rfl
            all_goals rfl
          refine ⟨max f_B N_B + 2, stR_B, locR_B, ?_, h_IH_inv_B, hQ_B⟩
          rw [heq, hblock_mono]; exact h_IH_B
      · -- exit: j = n_right
        -- body's second br_if 1 fires: exec 1 body = Break 1 → exec 2 loop = Break 0
        -- → exec 3 block = Fallthrough.  Q: stA.mem.read32(frame+12) = n_right.
        have hj_eq : j = n_right.toNat := Nat.le_antisymm hj_hi (Nat.not_lt.mp hlt_j)
        ...
        have hQ_j : stA.mem.read32 (frame + 12) = n_right := by
          ...
          apply UInt32.toNat_inj.mp
          simp
        exact ⟨3, stA, locA, h_block_exit_j, hI_save, Or.inr hQ_j⟩
    · -- exit: i = n_left
      -- body's first br_if 1 fires immediately: exec 1 body = Break 1 → exec 2 loop = Break 0
      -- → exec 3 block = Fallthrough.  Q: stA.mem.read32(frame+8) = n_left.
      have hi_eq : i = n_left.toNat := Nat.le_antisymm hi_hi (Nat.not_lt.mp hlt_i)
      ...
      have hQ_i : stA.mem.read32 (frame + 8) = n_left := by
        ...
        apply UInt32.toNat_inj.mp
        simp
      exact ⟨3, stA, locA, h_block_exit_i, hI_save, Or.inl hQ_i⟩
